						...
						let traceDescriptorParser = function (sentence) {
							...
							switch (key) {
								...
								case "RECEIVER_LOCATION":
									let testArray = value.split(" ");
									if (0 == testArray.length) {
										value = Number(testArray[0]);
									}
									else {
										value = [];
										for (var ii = 0; ii < testArray.length; ii++) {
											let tmpNumber = Number(testArray[ii]);
											if (NaN != tmpNumber) value.push(tmpNumber);
										}
									}
									break;
								default:
...
function buildArraysToPlot(header, data) {
	...
	let xpos = Array(header.ntraces);
	for (let nn = 0; nn < header.ntraces; nn++) {
		xpos[nn] = Math.sqrt(header.trace[nn].traceDescriptor['RECEIVER_LOCATION'].reduce((a, num) => a + num * num, 0));
	}
	let minDist = 0;
	for (let nn = 1; nn < header.ntraces; nn++) {
		minDist += Math.abs(xpos[nn] - xpos[nn - 1])
	}
	minDist /= header.ntraces;
	...
	for (let nn = 0; nn < header.ntraces; nn++) {
		...
		for (var ii = 0; ii < ns; ii++) {
			result[nn].y[ii] = t;
			result[nn].x[ii] = data[nn][ii] / (2 * maxamp) * minDist + xpos[nn];
			var temp = data[nn][ii];
			t += dt;
		}
	}
...
let seg2js = { createHeader, readHeader, readTraceData, buildArraysToPlot, buildWiggleData };
export default seg2js;
